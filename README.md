...
[![Docker](https://img.shields.io/badge/Docker-Ready-2496ED?logo=docker&logoColor=white)](docs/full-guide.md)
...
| 自动化与推送 | Docker、本地定时任务、FastAPI 服务和企业微信/飞书/Telegram/Discord/Slack/邮件推送 |
...
| 新闻搜索 | [Anspire](https://open.anspire.cn/)、[SerpAPI](https://serpapi.com/)、[Tavily](https://tavily.com/)、[Bocha](https://open.bocha.cn/)、[Brave](https://brave.com/search/api/)、[MiniMax](https://platform.minimaxi.com/)、SearXNG |
...
> Windows PowerShell 可使用 `Copy-Item .env.example .env` 复制配置模板。

### 基础配置

配置优先写入 `.env`。默认先选一个模型服务商并填写 API Key；需要多模型、图片识别、本地模型或高级路由时，再参考 [LLM 配置指南](docs/LLM_CONFIG_GUIDE.md)。

**AI 模型配置（至少配置一个）**

| 环境变量 | 说明 | 必填 |
|------------|------|:----:|
| `ANSPIRE_API_KEYS` | [Anspire](https://open.anspire.cn/) API Key，可用于大模型与联网搜索 | 可选 |
| `AIHUBMIX_KEY` | [AIHubMix](https://aihubmix.com/) API Key，可切换使用多类模型 | 可选 |
| `GEMINI_API_KEY` | Google Gemini API Key | 可选 |
| `ANTHROPIC_API_KEY` | Anthropic Claude API Key | 可选 |
| `OPENAI_API_KEY` | OpenAI 兼容 API Key（支持 DeepSeek、通义千问等） | 可选 |
| `OPENAI_BASE_URL` / `OPENAI_MODEL` | 使用 OpenAI 兼容服务时填写 | 可选 |
| `OLLAMA_BASE_URL` / `OLLAMA_MODEL` | 使用 Ollama 本地模型时填写 | 可选 |
...
| 环境变量 | 说明 | 必填 |
|------------|------|:----:|
| `TICKFLOW_API_KEY` | [TickFlow](https://tickflow.org/) 行情数据 | 可选 |
| `TUSHARE_TOKEN` | [Tushare](https://tushare.pro/) 数据接口 | 可选 |
| `LONGBRIDGE_APP_KEY` / `LONGBRIDGE_APP_SECRET` / `LONGBRIDGE_ACCESS_TOKEN` | [Longbridge](https://longbridge.com/) 行情与账户能力 | 可选 |
| AkShare / Pytdx / Baostock / YFinance | 多数场景无需额外 Key，按运行环境与市场自动降级使用 | 可选 |
...
| 环境变量 | 说明 | 必填 |
|------------|------|:----:|
| `ANSPIRE_API_KEYS` | [Anspire AI Search](https://aisearch.anspire.cn/)：中文内容检索；同一 Key 可复用为 Anspire 大模型 | 可选 |
...
Docker 部署同样读取环境变量或 `.env` 配置。镜像构建、定时任务、云服务器访问请参考 [完整指南](docs/full-guide.md)。
...
> 定时任务、服务部署和桌面客户端打包请分别参考 [完整指南](docs/full-guide.md) 与 [桌面端打包说明](docs/desktop-package.md)。
...
本项目为二次开发版本，后续改动以当前仓库维护目标为准。二次开发、分发或引用时，请注明原仓库来源：[ZhuLinsen/daily_stock_analysis](https://github.com/ZhuLinsen/daily_stock_analysis)。
